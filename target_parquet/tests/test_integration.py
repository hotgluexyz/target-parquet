...
class TestNonNullColumnsWithNullValues:
    ...
    def test_non_nullable_column_with_null_value(self, tmp_path):
        """Fields declared as {"type": "string"} are treated as nullable so that
        null values are stored correctly rather than corrupting the file."""
        messages = [
        ...
        run_target(messages)
        table = read_parquet_for_stream(tmp_path, "strict")
        assert table.num_rows == 1
        assert table.column("required_col")[0].as_py() is None
        assert table.schema.field("required_col").nullable
